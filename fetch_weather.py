...
#     # heatIndex/windChill are only present when relevant (hot/cold enough)
#     # Fall back to plain temperature
#     heat_index_f = celsius_to_fahrenheit(props["heatIndex"]["value"])
#     wind_chill_f = celsius_to_fahrenheit(props["windChill"]["value"])
#     if heat_index_f is not None:
#         feels_like_f = heat_index_f
...
#     # AirNow returns one entry per pollutant (O3, PM2.5, PM10, etc).
#     # The AQI reported to the public is the WORST of these: same convention the EPA uses.
#     return max(obs["AQI"] for obs in observations)


# def get_current_uv():
#     """Fetch the CURRENT hour's UV index from EPA Envirofacts (hourly forecast)."""
#     url = f"https://data.epa.gov/dmapservice/getEnvirofactsUVHOURLY/ZIP/{ZIP_CODE}/JSON"
#     response = requests.get(url)
#     response.raise_for_status()
#     data = response.json()
...
#     # Each entry's DATE_TIME looks like "Jul/10/2026 07 AM": find the matching the current date and hour
#     now = datetime.now()
#     for entry in data:
#         entry_time = datetime.strptime(entry["DATE_TIME"], "%b/%d/%Y %I %p")
#         if entry_time.date() == now.date() and entry_time.hour == now.hour:
#             return float(entry["UV_VALUE"])

#     # No entry overnight, since EPA doesn't bother listing hours where UV is basically zero (no sun).
#     return 0.0
...
#     print()
#     print("UV raw response (EPA) -- inspect this to confirm field names:")
#     import json
#     uv_url = f"https://data.epa.gov/dmapservice/getEnvirofactsUVDAILY/ZIP/{ZIP_CODE}/JSON"
#     uv_response = requests.get(uv_url)
#     print(json.dumps(uv_response.json(), indent=2))